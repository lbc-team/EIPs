...
TEIP草案描述了以太坊命名服务的详细信息、协议建议和可为服务和资源标识符提供易于理解的短名称灵活解析的ABI定义。这允许用户和开发人员引用易于理解且易于记忆的名称，并允许在基础资源（合同，内容寻址的数据等）发生更改时根据需要更新这些名称。

域名的目的是提供稳定的、人类可读的标识符，该标识符可用于指定网络资源。这样，用户输入一个好记的字符串，如“vitalik.wallet”或“www.mysite.swarm” ，就可以定向到合适的资源。名称和资源之间的映射可能会变化，因此用户可能会更改钱包，网站可能更改主机，或者大量文档会更新为新版本，而域名不会更改。此外，域无需指定单个资源；不同的记录类型允许同一域引用不同的资源。例如，浏览器可以通过获取其地址记录来将“mysite.swarm”解析为其服务器的IP地址，而邮件客户端可以通过获取其邮件交换器记录来将相同地址解析为邮件服务器。
