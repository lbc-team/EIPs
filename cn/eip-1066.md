...
Since ERC1066 status codes are finite and known in advance, we can leverage [ERC1444](eip-1444.html) to provide global, human-readable sets of status messages. These may also be translated into any language, differing levels of technical detail, added as `revert` messages, natspecs, and so on.
...
We also see a desire for this [in transactions](eip-658.html), and there's no reason that these status codes couldn't be used by the EVM itself.
