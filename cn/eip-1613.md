...
Solutions such as described in [EIP-1077](eip-1077.html) could allow transactions from addresses that hold no ETH.

The gas stations network is an [EIP-1077](eip-1077.html) compliant effort to solve the problem by creating an incentive for nodes to run gas stations, where gasless transactions can be "fueled up". 
