...
4. Allow the compatibility with token standard such as [ERC-20](eip-20.html) or other new standards([EIP-777](eip-20.html)) and item standard such as [EIP-721](eip-20.html)
5. Create massive potential for interoperability within Ethereum echo systems and other system.
6. Allow setting voting deadline, allow determine on single or multiple options. Allow requiring voting orders. (trade-off is interface complexity, we might need [ERC-20](eip-20.html) approach and later a [EIP-777](eip-20.html) for advanced voting) 
...
3. Should the voting support proxy(e.g [EIP-897](eip-897.html), [EIP-1167](eip-897.html)) and migration? What are potential security concerns
