...
| 作者 | 状态 | 类型 | 分类 | 创建时间 |
| --- | --- | --- | --- | --- |
| Nick Johnson <arachnid@notdot.net>| 最终 | Standards Track| ERC | 2016-04-04|


## 摘要

以太坊命名服务（ENS）类似于互联网的域名服务（DNS），DNS转换的是IP地址，ENS转换的是以太坊地址，如可以把以太坊地址转换为 `mycoin.eth` 。
如何使用ENS及基于ENS，可以查阅[ENS中文文档](https://learnblockchain.cn/docs/ens/)

此EIP草案描述了以太坊命名服务（ENS）的细节，是一个协议标准和ABI定义，其提供了把简短、人类可读的域名映射到服务和资源标识符的灵活解析方案。它允许用户和开发者以人类可读的、便捷的方法来记住域名，并使这些域名在其基础资源（合约、内容处理数据等等）发生改变时能按需及时被更新。
...
域名的目的是提供能被用于指定网络资源的、稳定的、人类可读的标识符。通过这种方法，用户能够输入一个可记的字符串，例如'vitalik.wallet'或'www.mysite.swarm'，并且能被定位到正确的资源上。名字和资源之间的映射可能会改变，所以一个用户可以改变钱包、一个网站可能改变主机、或一个swarm文档可能被更新到一个新的版本，而不需要域名变化。进一步来说，一个域名不需要指定一个资源；不同的记录类型允许相同的域名来指代不同的资源。举例，一个浏览器可能会根据获取它的A（地址）记录来把'mysite.swarm'解析到它服务器的IP地址，而一个邮件客户端可能会根据获取它的MX(mail exchanger) 记录来把相同的地址解析到一个邮件服务器。
