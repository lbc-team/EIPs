...
4. `tokensReceived` 可以通过hook函数可以做到在一个交易里完成 发送代币和通知合约接受代币，而不像 [ERC20] 必须通过两次调用（`approve`/`transferFrom`）来完成。
...
2. 可以通过部署实现  `tokensReceived` 的代理合约来兼容没有实现`tokensReceived` 函数的钱包。
...
[erc1820-set]: https://learnblockchain.cn/docs/eips/eip-1820.html#set-an-interface-for-an-address
